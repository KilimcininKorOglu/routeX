...
func TestRule_IsMatch_RegEx(t *testing.T) {
	tests := []struct {
		...
		{"example", "example.com", true},     // partial match
		{"^example$", "example.com", false},  // exact match
		{"(?i)EXAMPLE", "example.com", true}, // case insensitive (already enabled)
...
func TestRule_IsMatch_RegEx_Invalid(t *testing.T) {
	...
	// Verify that Compile returns an error
