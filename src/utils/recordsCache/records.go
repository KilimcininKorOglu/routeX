...
type Records struct {
	...
	// Separate maps instead of map[string]interface{}
	addresses map[string][]*Address
	aliases   map[string]*Alias

	// Reverse index: alias -> []domains that point to it
...
func (r *Records) AddAlias(domainName, alias string, ttl uint32) {
	...
	// Remove old reverse alias if one existed
...
// GetAliases returns all domains that point to the given one (directly or transitively)
...
func (r *Records) GetAddresses(domainName string) []*Address {
	...
	for {
		if addresses, ok := r.addresses[domainName]; ok && len(addresses) > 0 {
			// Filter out expired addresses (read-only, no deletion)
			var valid []*Address
...
func (r *Records) ListKnownDomains() []string {
	...
	// Collect unique domains from both maps
...
// cleanupRecords removes expired records
...
// StartCleanup starts background cleanup at the given interval
