...
func (ipt *IPTables) GetCurrentRules() (map[string]map[string][]Rule, error) {
	...
	for _, line := range lines {
		...
		case ':':
			...
			// Find the first space to extract the chain name
		...
		case '-':
			...
			switch fields[0][1] {
			case 'A': // Append
				// fields[2:] - rule without the operator
...
// splitFields splits a line into fields by whitespace, returning slices of the original buffer
